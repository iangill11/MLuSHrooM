...
#include "shroomFLAGS.h"
  ...
  double precision, parameter :: CFL = 0.50d0
...
module MPItools
  ...
  integer, parameter :: fftKyID=0   ![ ky rank ID of processes in fftCOMM.
#endif
...
![ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  ...
  implicit none
#if (ioMode > 0)
  character(len=4)  :: cN1L,cN2L,cN1G,cN2G,cN1off,cN2off
  character(len=11) :: cDimsL,cDimsG,cOffsets 
  integer*8         :: bpVarID
  integer           :: i
#endif
...
module FFTmodule
  ...
  logical, parameter :: outputWisdom = .false.
...
![ endif for if (FFTmode==0) then else.
  ...
  implicit none
  integer                          :: fftErrFlag
  character(C_CHAR), dimension(50) :: wisdomFile
  character(len=50)                :: wisdomFileF
  character(len=4)                 :: cN1,cN2,cProcX,cProcY,cThreads,cMode
  ...
  if (fftMember) then
    ...
    if (myRank == 0) then
      ...
      wisdomFile = trim(wisdomFileF)//C_NULL_CHAR
      fftErrFlag = fftw_import_wisdom_from_filename(wisdomFile)
    ...
    if (lockxa /= Nekxa(2)) then
      write(*,'(" Shroom and FFTW use different MPI decomposition. myRank=",i3," | lockxa=",i3," | Nekxa=",i3)') &
    ...
    fftM = NyaG
    fftN = NxaG
    plan2Da_c2r = fftw_mpi_plan_dft_c2r_2d(fftN, fftM, funka, funRa, fftCOMM, FFTW_ESTIMATE)
    plan2Da_r2c = fftw_mpi_plan_dft_r2c_2d(fftN, fftM, funRa, funka, fftCOMM, FFTW_ESTIMATE)

    ![ Save wisdom if there is none available
    if (outputWisdom) then
      call fftw_mpi_gather_wisdom(fftCOMM)
      if (myRank == 0) then
        fftErrFlag = fftw_export_wisdom_to_filename(wisdomFile)
...
![ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  ...
  ![ Assuming de-aliased data is distributed across kx only, we will transfer it
  ![ to a larger alased array that is also distributed across kx.
  ![ This involves communication in 1D, via fftCOMM.
  implicit none
  integer              :: intBuf
...
![ else for if (FFTmode==0) then else.
  ...
  allocLocal = fftw_mpi_local_size_many_1d(fftN,howMany,xyCOMMs(2), &
               FFTW_BACKWARD,FFTW_ESTIMATE,lockxa,lockxa0,locxa,locxa0)
  ...
  plan1Da_c2r_x = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                  ...
                  funka,funkya,xyCOMMs(2),FFTW_BACKWARD,FFTW_ESTIMATE)
  plan1Da_r2c_x = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                  FFTW_MPI_DEFAULT_BLOCK,FFTW_MPI_DEFAULT_BLOCK, &
                  funkya,funka,xyCOMMs(2),FFTW_FORWARD,FFTW_ESTIMATE)
  ...
  allocLocal = fftw_mpi_local_size_many_1d(fftN,howMany,xyCOMMs(1), &
               FFTW_BACKWARD,FFTW_ESTIMATE,lockya,lockya0,locya,locya0)

  if (lockya /= Nxa(1)) then
    write(*,'(" Shroom and FFTW use different MPI decomposition. myRank=",i3," | lockya=",i3," | Nya=",i3)') &
      myRank,lockya,Nxa(1)
    call abortSimulation(' ')
  endif
  ...
  plan1Da_c2r_y = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                  FFTW_MPI_DEFAULT_BLOCK,FFTW_MPI_DEFAULT_BLOCK, &
                  funkyaTc,funRa,xyCOMMs(1),FFTW_BACKWARD,FFTW_ESTIMATE)
  plan1Da_r2c_y = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                  FFTW_MPI_DEFAULT_BLOCK,FFTW_MPI_DEFAULT_BLOCK, &
                  funRa,funkyaTc,xyCOMMs(1),FFTW_FORWARD,FFTW_ESTIMATE)
  ...
  if (initialOp == 0) then
    ...
    d1      = Nx(2)
    allocLocal = fftw_mpi_local_size_many_1d(fftN,howMany,xyCOMMs(1), &
                 FFTW_FORWARD,FFTW_ESTIMATE,locy,locy0,locky,locky0)
    ...
    plan1D_r2c_y = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                   FFTW_MPI_DEFAULT_BLOCK,FFTW_MPI_DEFAULT_BLOCK, &
                   funR,funkyTc,xyCOMMs(1),FFTW_FORWARD,FFTW_ESTIMATE)
    plan1D_c2r_y = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                   FFTW_MPI_DEFAULT_BLOCK,FFTW_MPI_DEFAULT_BLOCK, &
                   funkyTc,funR,xyCOMMs(1),FFTW_BACKWARD,FFTW_ESTIMATE)
    ...
    d1      = Nekx(1)
    allocLocal = fftw_mpi_local_size_many_1d(fftN,howMany,xyCOMMs(2), &
                 FFTW_FORWARD,FFTW_ESTIMATE,locx,locx0,lockx,lockx0)
    ...
    plan1D_r2c_x = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                   FFTW_MPI_DEFAULT_BLOCK,FFTW_MPI_DEFAULT_BLOCK, &
                   funky,funk,xyCOMMs(2),FFTW_FORWARD,FFTW_ESTIMATE)
    plan1D_c2r_x = fftw_mpi_plan_many_dft(1,m1D,howMany, &
                   FFTW_MPI_DEFAULT_BLOCK,FFTW_MPI_DEFAULT_BLOCK, &
                   funk,funky,xyCOMMs(2),FFTW_BACKWARD,FFTW_ESTIMATE)
  endif
...
![ else for if (useMPI>0) then else. 
  ...
  plan2Da_c2r = fftw_plan_dft_c2r_2d(NxaG, NyaG, funka, funRa, FFTW_ESTIMATE)
  plan2Da_r2c = fftw_plan_dft_r2c_2d(NxaG, NyaG, funRa, funka, FFTW_ESTIMATE)
  ...
  if (initialOp == 0) then
    ...
    ![ The order of the sizes has to be reversed here.
    plan2D_c2r = fftw_plan_dft_c2r_2d(NxG, NyG, funk, funR, FFTW_ESTIMATE)
    plan2D_r2c = fftw_plan_dft_r2c_2d(NxG, NyG, funR, funk, FFTW_ESTIMATE)
  endif
...
![ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
#if (ADJUSTdt > 0)
  function newDt(vxIn,vyIn,dxIn,dyIn,dtIn)
  ![ Check CFL condition (in real space for now) and adjust the time step.
  ...
  double precision             :: vxIn(:,:),vyIn(:,:),dxIn,dyIn,dtIn
  double precision, parameter  :: tol=0.50d0, chng=0.50d0
  double precision             :: maxrDt, minDt, propDt

  maxrDt = -1.0d12
  do lcx=1,Nxa(2); do lcy=1,Nya(1)
    maxrDt = max(maxrDt,abs(vxIn(lcy,lcx))/dxIn,abs(vyIn(lcy,lcx))/dyIn)
  enddo; enddo
  minDt = CFL/maxrDt
...
![ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  ...
  if (myrank==0) &
    write(*,'(" User requested  NkxG=",i4, " and NkyG=",i4, " distinct wavenumbers (absolute magnitude).")') NkxG, NkyG
...
![ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  ...
  gatherCount = Nekx(1)*Nekx(2)
  call MPI_Gather(fkIn, gatherCount, MPI_DOUBLE_COMPLEX, &
                  funkyG, gatherCount, MPI_DOUBLE_COMPLEX, &
                  fftKyID, xyCOMMs(1), errFlagMPI)
...
![ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  ...
  scatterCount = Nekx(1)*Nekx(2)
  call MPI_Scatter(funkyG, scatterCount, MPI_DOUBLE_COMPLEX, &
                   fkOut, scatterCount, MPI_DOUBLE_COMPLEX, &
                   fftKyID, xyCOMMs(1), errFlagMPI)
#else
...
program shroom
  ...
  do while (timeStepMore)
    ...
    if (abs(simTime - dble(dtAdjusts+1)*tRateAdjustDt) <= tTol) then
      ...
      dt = newDt(-gRa_y,gRa_x,dxa,dya,dt)
