...
phi_data = np.transpose(phi_data, (2, 1, 0)).astype(np.float32) 
...
resized_x_coords = transform(x_coords).squeeze()
resized_y_coords = transform(y_coords).squeeze()
...
# Apply the transform (Resize) to phi_data
phi_data = transform(phi_data) 
print("Shape of data:", phi_data.shape)
phi_data = rescale(phi_data)
x_train = phi_data[0:ntrain, :, :]
y_train= phi_data[1:ntrain+1, :, :]

x_vali = phi_data[ntrain+100:ntrain+200, :, :]
y_vali= phi_data[ntrain+101:ntrain+201, :, :]

x_test = phi_data[ntrain+300:ntrain+400, :, :]
y_test= phi_data[ntrain+301:ntrain+401, :, :]
''''
