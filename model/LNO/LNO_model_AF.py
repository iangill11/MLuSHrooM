...
class LNO2d(nn.Module):
    def __init__(self, in_channels, width1, width2, width3, modes11, modes12, modes21, modes22):
        ...
        # Fully connected layers depend on in_channels + 2 (grid features)
        self.fc1 = nn.Linear(in_channels + 2, self.width1)
        self.fc2 = nn.Linear(in_channels + 2, self.width2)

        # Convolution layers for steady-state responses
        ...
        # Convolution layers for transient responses
        ...
        self.fc4 = nn.Linear(self.width1, 128)  # Transient feature mapping
        self.fc5 = nn.Linear(self.width2, 128)  # Steady-state feature mapping

        # Output layers
        self.fc7 = nn.Linear(128, 1)  # Transient output
        self.fc8 = nn.Linear(128, 1)  # Steady-state + W output
...
# ====================================  
    def forward(self, f):
        ...
        # Concatenate the input tensor with the grid
        f = torch.cat((f, grid), dim=-1)  # Adds grid features to input
    
        # Transient part
        f1 = self.fc1(f)  # Fully connected layer for transient branch
        f1 = f1.permute(0, 3, 1, 2)  # (batch, channels, height, width)
        ...
        x1 = x1.permute(0, 2, 3, 1)  # Back to (batch, height, width, channels)
        x1 = self.fc4(x1)  # Transient features to 128
        x1 = torch.sin(x1)
        x1 = self.fc7(x1)  # Transient output
    
        # Steady-state part + W part
        f2 = self.fc2(f)  # Fully connected layer for steady-state branch
        f2 = f2.permute(0, 3, 1, 2)  # (batch, channels, height, width)
        ...
        x23 = x23.permute(0, 2, 3, 1)  # Back to (batch, height, width, channels)
        x23 = self.fc5(x23)  # Steady-state features to 128
        x23 = torch.sin(x23)
        x23 = self.fc8(x23)  # Steady-state output
    
        # Final output: combine transient and steady-state parts
        return x1 + x23
